...
-->
本页面概述了系统管理员可以在不同的 Kubernetes 组件上使用的各种特性开关。
{{% /capture %}}
...
-->
特性开关是一组用来描述 alpha 或者实验特性的键值对。
管理员可以在每个组件的命令行使用 `--feature-gates` 参数来开启或者关闭某个特性。
下面的表格是您可以在不同的 Kubernetes 组件中可以设置的特性开关的总结。
...
- "开始版本" 这列包含了什么时间引入的 Kubernetes
- 如果 "结束版本" 这列不为空，则包含了您仍然可以使用这个特性开关的最后的 Kubernets 版本。
...
-->
一个特性可能位于 *Alpha*、*Beta* 或 *GA* 阶段。
*Alpha* 特性意味着：

* 默认是禁用的。
* 可能有 bug。开启这个特性可能会暴露一些 bug 。
* 可能在任何时候没有通知就会放弃对这个特性的支持。
* API 可能在后续软件版本中有不兼容的改变。
* 由于 bug 风险的提高和缺少长期支持，推荐只在短期测试集群中使用。
